...
# %%
def load_results(version="1.4.0"):
...
# %%
results =load_results(version="1.5.0")
...
# %% [markdown]
# Ups! I accidentally did the less important sparsity values first!
...
# %%
dataset = SyntheticBinaryValued(num_samples=1000, num_features=6, sparsity=0.5)

# %%
model = ToyAutoencoder(6, 2, final_bias=True)

# %%
model.load_state_dict({k:torch.tensor(v) for k,v in weights[0.75][0][-1].items()})

# %%
for i in range(10):
    print(f"Sample {i}")
    print(dataset[i])
    print(model(dataset[i].unsqueeze(0)))
